...
### カスタム形式管理機能の強化
- **カスタム形式削除機能**: 設定画面から不要なカスタム形式を削除可能
- **Custom Formatsタブ**: 全カスタム形式を一覧管理
- **デフォルト形式の編集**: 標準形式も編集可能（削除は不可）

### AI生成・翻訳機能の完全実装
- **Split to Tags（AI翻訳）**: テキストを分解後、各タグをAIで自動翻訳（英⇄日）
- **AI Generate**: 入力テキストから形式別（SDXL/Flux/ImageFX）のタグを生成
- **完全同期**: 英語・日本語のタグが完全に連動、片側編集で自動翻訳
- **インライン編集**: タグをクリックして直接編集可能
- **カテゴリ自動判定**: AI生成時にタグのカテゴリを自動判定・色分け

### 形式切り替え機能の改善
- **独立した形式選択**: AI Format（生成用）とFinal Output Format（表示用）を分離
- **システムプロンプト編集**: 各形式の生成ルールをカスタマイズ可能
- **カスタム形式追加**: ユーザー独自の形式を自由に追加
